...
longest life. Table \@ref(tab:surgery) contains the hypothetical number of
...
as the difference between the two outcomes $$(\#eq:ice)
...
individual causal effect $\tau_i$ is estimated by $$(\#eq:hattau)
...
Figure \@ref(fig:horrible) shows the values of the observed and imputed
...
Table \@ref(tab:surgery) is 0.9, which allows for fairly large differences
...
for the potential outcomes: $$(\#eq:bivextra)
...
these estimates right away in Equation \@ref(eq:bivextra). Rather we equate
...
Figure \@ref(fig:rhodens) shows the imputed data (red) against a backdrop
...
$\rho = 0.99$ (bottom). Data from Table \@ref(tab:surgery).<span
data-label="fig:fan1">](fig/ch8_ice_parallelplot1-1){width="\maxwidth"}

Figure \@ref(fig:fan1) displays imputations by patients for three different
...
`mice`. The method in Section \@ref(sec:fcsprior)
