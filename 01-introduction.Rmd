...
`plot(fit)` to study the quality of the model.
In practice, it is cumbersome to supply the
`na.action()` function each time. We can
change the setting in `options` as
...
against the observed data, so we use
`predict()` to calculate the predicted values,
and add these to the data to prepare for plotting.

```{r air4}
```

Argg... that doesn’t work either. The error message tells us that the
two datasets have a different number of rows. The
`airquality` data has 153 rows, whereas there
are only 116 predicted values. The problem, of course, is that there are
missing data. The `lm()` function dropped any
incomplete rows in the data. We find the indices of the first six cases
by
...
Figure \@ref(fig:air9) plots the predicted against the observed values.
...
```{r plotair9}
```
![Predicted versus measured ozone levels for the observed (left, blue)
and missing values (right, red).<span
data-label="fig:air9">](fig/ch1_plotair9-1){width="\maxwidth"}
...
```{r plotmeanimp}
```
![Mean imputation of `Ozone`. Blue indicates the observed data, red 
indicates the imputed values.
<span data-label="fig:meanimp">](fig/ch1_plotmeanimp-1){width="\maxwidth"}

Mean imputation distorts the distribution in several ways. Figure
\@ref(fig:meanimp) displays the distribution of
...
```{r plotregimp}
```
![Regression imputation: Imputing `Ozone` from the regression line.<span
data-label="fig:regimp">](fig/ch1_plotregimp-1){width="\maxwidth"}
...
Figure \@ref(fig:regimp) shows the result. The imputed values correspond to
...
```{r plotsri}
```
![Stochastic regression imputation of `Ozone`.<span
data-label="fig:sri">](fig/ch1_plotsri-1){width="\maxwidth"}
...
`seed` argument makes the solution reproducible. Figure \@ref(fig:sri) 
...
linear model for non-negative data. In fact, Figure \@ref(fig:air9) shows
...
\@ref(ex:sri)). The main idea to draw from the residuals is very powerful,
and forms the basis of more advanced imputation techniques.
...
```{r locf2}
```
![Imputation of `Ozone` by last observation
carried forward (LOCF).<span
data-label="fig:locf">](fig/ch1_locf2-1){width="\maxwidth"}

Figure \@ref(fig:locf) plots the results of the first 80 days of the
...
### Summary {#sec:simplesummary}
  ...
  : Overview of assumptions made by ad-hoc methods.<span
  data-label="tab:simple">
...
(8,5) (1,3) (3,1.5) (3,3) (3,4.5) (5,1.5) (5,3) (5,4.5) (7,3)
(1.35,3)(1,0)1.3
(1.35,3)(1,1)1.3
(1.35,3)(1,-1)1.3
(3.35,1.5)(1,0)1.3
...
(5.35,4.5)(1,-1)1.3
(5.35,1.5)(1,1)1.3
(0,0)(2,1)Incomplete data
(2,0)(2,1)Imputed data
(4,0)(2,1)Analysis results
(6,0)(2,1)Pooled result
...
Continuing with the `airquality` dataset, it
is straightforward to apply multiple imputation. The following code
imputes the missing data twenty times, fits a linear regression model to
predict `Ozone` in each of the imputed
datasets, pools the twenty sets of estimated parameters, and calculates
the Wald statistics for testing significance of the weights.
...
`options(na.action = na.omit)`, fitting the
same model to the complete cases can be done by
...
```{r plotairmi}
```
![Multiple imputation of `Ozone`. Plotted are
the imputed values from the first imputation.<span
data-label="fig:airmidist">](fig/ch1_plotairmi-1){width="\maxwidth"}

Figure \@ref(fig:airmidist) shows the distribution and scattergram for the
...
\@ref(fig:sri) are now gone since the imputation method used observed data
...
nature of the relation between `Ozone` and
`Solar.R`. All in all, the red points look as
if they could have been measured if they had not been missing. The
reader can easily recalculate the solution and inspect these plots for
the other imputations.

```{r air10}
```
![Multiple imputation of `Ozone`. Plotted are
the observed values (in blue) and the multiply imputed
values (in red).<span
data-label="fig:airmi">](fig/ch1_air10-1){width="\maxwidth"}

Figure \@ref(fig:airmi) plots the completed `Ozone` data. The 
...
1.  *Reporting practice*. What are the reporting practices in your
    field? Take a random sample of articles that have appeared during
    the last 10 years in the leading journal in your field. Select only
    those that present quantitative analyses, and address the following
    topics:

    1.  Did the authors report that there were missing
        data?
    ...
    5.  Can you detect a trend over time in reporting
        practice?

    6.  Would the editors of the journal be interested in your
        findings?
    (\#ex:report)

2.  *Loss of information*. Suppose that a dataset consists of 100
    cases and 10 variables. Each variable contains 10% missing values.
    What is the largest possible subsample under listwise deletion? What
    is the smallest? If each variable is MCAR, how many cases will
    remain?(\#ex:caseslost)

3.  *Stochastic regression imputation*. The correlation of the data in
    Figure \@ref(fig:sri) is equal to 0.33. This is relatively low compared
    to the other correlations reported in
    Section \@ref(sec:simplesolutions). This seems to contradict the
    statement that stochastic regression imputation does not bias the
    correlation. Could this low correlation be due to random variation?

    1.  Rerun the code with a different `seed` value. What is
        the correlation now?

    2.  Write a loop to apply apply stochastic regression
        imputation with the `seed` increasing
        from 1 to 1000. Calculate the regression weight and the
        correlation for each solution, and plot the histogram. What are
        the mean, minimum and maximum values of the correlation?

    3.  Do your results indicate that stochastic regression
        imputation alters the correlation?
    (\#ex:sri)

4.  *Stochastic regression imputation (continued)*. The largest
    correlation found in the previous exercise exceeds the value found
    in Section \@ref(sec:regimp). This seems odd since the correlation of
    the imputed values under regression imputation is equal to 1, and
    hence the imputed data have a maximal contribution to the
    overall correlation.

    1.  Can you explain why this could happen?

    2.  Adapt the code from the previous exercise to test
        your explanation. Was your explanation satisfactory?
...
5.  *Nonlinear model*. The model fitted to the
    `airquality` data in Section
    \@ref(sec:miexample) is a simple linear model. Inspection of the
    residuals reveals that there is a slight curvature in the average of
    the residuals.

    1.  Start from the completed cases, and use
        `plot(fit)` to obtain
        diagnostic plots. Can you explain why the curvature shows
        up?

    2.  Experiment with solutions, e.g., by transforming
        `Ozone` or by adding a quadratic term
        to the model. Can you make the curvature disappear? Does the
        amount of explained variance increase?

    3.  Does the curvature also show up in the imputed data? If
        so, does the same solution work? Hint: You can assess the
        $j^\mathrm{th}$ fitted model by
        `getfit(fit, j)`, where
        `fit` was created by
        `with(imp,...)`.
