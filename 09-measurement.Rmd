...
Section \@ref(sec:sensitivity) continues with the same data, and shows how
a simple sensitivity analysis can be done. Section \@ref(sec:prevalence)
illustrates how multiple imputation can be used to estimate overweight
prevalence from self-reported data. Section \@ref(sec:codingsystems) shows
...
indicator method (cf. Section \@ref(sec:indicator)). The message is that
...
Figure \@ref(fig:c85influx) is the influx-outflux pattern of Leiden 85+
...
selection strategy of Section \@ref(sec:predictors). In order to apply this
...
is about right (cf. Section \@ref(sec:predictors)), so I decided to accept
...
practiced in Section \@ref(sec:toomany). More importantly, the new solution
is much better. To illustrate the latter, take a look at Figure
\@ref(fig:blindvsquickpred).
...
`mice`. Table \@ref(tab:c85hazardcor) lists the
...
\@ref(sec:MCARreprise)). While this is often a good starting assumption, it
...
\@ref(sec:whenignorable) for a more elaborate discussion of these
strategies.
...
The missingness is strongly related to survival. Figure \@ref(fig:c85km)
...
Table \@ref(tab:c85rdist) indicates that BP was measured less frequently
...
Table \@ref(tab:c85pmiss) contains the proportion of persons for which BP
...
Table \@ref(tab:c85pmiss)), selection of the complete cases could blur the
...
Section \@ref(sec:nonignorable) advocated the use of simple adjustments to
the imputed data as a way to perform sensitivity analysis. Table
\@ref(tab:delta) lists possible values for an offset $\delta$, together
...
Table \@ref(tab:c85diff) provides the differences in means between the
...
Table \@ref(tab:c85sensresults) provides the hazard ratio estimates under
...
Section \@ref(sec:nonignorableoverview) discussed shift, scale and shape
...
\@ref(fig:plotbmi) is drawn from data of @KRUL2010. Self-reported BMI is on
...
\@ref(fig:plotbmi) persons in the white area labeled “1” are obese
...
Figure \@ref(fig:plotexplain) plots 
the data of Figure \@ref(fig:plotbmi) in a
...
quadrant numbers used in Figures \@ref(fig:plotbmi) 
and \@ref(fig:plotexplain)
correspond to identical subdivisions in the data.

The “true obese” in Figure \@ref(fig:plotexplain) lie in quadrants 1 and 4.
...
We apply predictive mean matching (cf. Section \@ref(sec:pmm)) to impute
`hm` and `wm` in the
...
associated confidence intervals as in Sections \@ref(sec:threesources) and
\@ref(sec:inference).
...
self-reported data. The plot is identical to Figure \@ref(fig:plotbmi),
...
2 columns: `YA` and
`YB`. Figure \@ref(fig:walkingpattern1) shows
that the missing data pattern is unconnected (cf. Section
\@ref(sec:patternoverview)), with no observations linking
`YA` to `YB`. There
...
Figure \@ref(fig:walkingtracenolink) contains the trace plot of 50
...
$\hat\theta_\mathrm{BA}$. In particular, in Section \@ref(sec:equating) we
...
\@ref(fig:walkingtracenolink) illustrates that it is extremely unlikely
...
`mice` as the dataset
`walking`. Figure \@ref(fig:walkingpattern2)
...
\@ref(fig:walkingtracelink)). After the first few iterations, the trace
...
Figure \@ref(fig:walkingthetaAB) plots the traces of MICE algorithm, where
...
equal to 0.807 (cf.Section \@ref(sec:equating)). The difference between the
...
4.  *Heterogeneity*. Kendall’s $\tau$ in the source E is 0.57 (cf.
    Section \@ref(sec:untenable)). The average of the sampler is slightly
    lower (Figure \@ref(fig:walkingtracelink)). Adapt the
