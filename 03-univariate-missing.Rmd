...
Figure \@ref(fig:whiteside)a plots the observed data. More gas is needed in
...
$5.49-0.29 \times 5 = 4.04$. Figure \@ref(fig:whiteside)b shows where the
...
regression line. Figure \@ref(fig:whiteside)c illustrates five such drawn
...
Figure \@ref(fig:whiteside)d shows five sampled regression lines calculated
...
Figure \@ref(fig:whiteside)e shows the same data, but now flagged according
...
regression line for the insulated houses. Figure \@ref(fig:whiteside)e
...
Figure \@ref(fig:whiteside)f illustrates an alternative method to create
...
Algorithm \@ref(alg:norm) is adapted from @RUBIN1987 [p. 167], and is
...
through resampling the data [@EFRON1993]. Algorithm \@ref(alg:normboot)
...
Table \@ref(tab:linmody) summarizes the results for the situation where we
...
This result confirms the problems already noted in Section \@ref(sec:true).
...
it yields the shortest confidence interval (cf. Section \@ref(sec:when)).
...
change in the function to create the missing data. Table \@ref(tab:linmodx)
...
short, but less than in Table \@ref(tab:linmody). Methods
`norm`, `norm.boot`
...
mechanisms, such as those discussed in Section \@ref(sec:generateuni). Data
...
Figure \@ref(fig:missingness) displays the probability of being missing
...
\@ref(fig:threemars) displays the distributions of $Y_2$ under the three
...
Tables \@ref(tab:linmody) and 
\@ref(tab:linmodx) show that methods
...
with increasing sample size (Exercise \@ref(ex:sampling)), so for estimates
...
Figure \@ref(fig:hc) plots the data for Dutch boys aged 1–2 years. Due to
...
Figure \@ref(fig:hcimp) is the 
equivalent of Figure \@ref(fig:hc), but now
...
Figure \@ref(fig:pmm) is an illustration of the method using the
...
Algorithm \@ref(alg:pmm) provides the steps used in predictive mean
...
need to draw $\beta$ along the same lines as Algorithm \@ref(alg:normboot).
...
\@ref(tab:linmody) and 
\@ref(tab:linmodx) for predictive mean matching for
...
undercoverage for missing data in $x$. Note that Table \@ref(tab:pmm) in
...
Algorithm \@ref(alg:cart) describes the major steps of an algorithm for
...
Algorithm \@ref(alg:binary) provides the steps for an approximate Bayesian
...
difficulties in Algorithm \@ref(alg:binary). Prior to estimating
...
\@ref(alg:binary). The estimate of $V$ will be very large as a result of
...
3.  Use penalized regression with Jeffreys prior in step 2 of
    Algorithm \@ref(alg:binary) [@FIRTH1993; @HEINZE2002].
...
`cart` (cf. Section \@ref(sec:cart)) and to
latent class models for categorical data (cf. Section \@ref(sec:JM)).
...
Algorithm \@ref(alg:KMIB) is based on the KIMB method proposed by @HSU2006.
...
$P(Y,R)$ as $$P(Y,R) = P(Y)P(R|Y). (\#eq:selection)$$ The selection
...
\@ref(eq:selection) this model only reverses the roles of $Y$ and $R$, but
...
pressure in Table \@ref(tab:c85nmar). See text for explanation.<span
data-label="c85:fignmar">](fig/ch3_c85sensfig-1){width="\maxwidth"}

Figure \@ref(c85:fignmar) is an illustration of the posited missing data
...
Sections \@ref(sec:selectionmodel)–
\@ref(sec:convert) provide different,
...
1.  *MAR*. Reproduce Table \@ref(tab:linmody) and Table
    \@ref(tab:linmodx) for MARRIGHT, MARMID and MARTAIL missing data
    mechanisms of Section \@ref(sec:generateuni).
...
2.  *Parameter uncertainty*. Repeat the simulations of Section
    \@ref(sec:linearnormal) on the `whiteside`
