...
Figure \@ref(fig:mcarmar) displays the distribution of $Y_\mathrm{obs}$ and
...
$$E(\hat Q|Y) = Q  (\#eq:unbiasedness)$$ The explanation of
...
formula is $$E(U|Y) \geq V(\hat Q|Y) (\#eq:confidenceproper)$$ where
...
variances as $$\bar U = \frac{1}{m}\sum_{\ell=1}^m \bar U_\ell
  (\#eq:within)$$ where the term $\bar U_\ell$ is the
...
$$B = \frac{1}{m-1}\sum_{\ell=1}^m (\hat Q_\ell-\bar Q)(\hat Q_\ell-\bar Q)'
  (\#eq:between)$$ where $\bar Q$ is calculated by Equation
\@ref(eq:poolQ).
...
$$\lambda = \frac{B+B/m}{T} (\#eq:lambda)$$ can be interpreted as
...
imputation can be written concisely as $$\nu =
  \frac{\nu_\mathrm{old} \nu_\mathrm{obs}}
  {\nu_\mathrm{old}+\nu_\mathrm{obs}} (\#eq:newnu)$$ The quantity
...
\@ref(eq:within), 
\@ref(eq:between) and 
\@ref(eq:poolT), respectively. Column
