...
implementation is still in the developmental state” [@DEMPSTER1983 p.
8].
...
(cf. Section \[sec:changingperspective\]). Nowadays multiple imputation
...
![Multiple imputation at age 40. Number of publications (log) on
...
14, 2018).<span
data-label="fig:publications">](fig/ch2_publications-1){width="\maxwidth"}

Figure \[fig:publications\] contains three time series with counts on
...
by edit and imputation techniques.
  ...
  : Examples of reasons for missingness for combinations of
  intentional/unintentional missing data with item/unit
  nonresponse.<span data-label="tab:intentional">

Table \[tab:intentional\] cross-classifies both distinctions, and
...
Section \[sec:MCAR\] introduced MCAR, MAR and MNAR. This section
...
$$\begin{aligned}
  ...
  \mathrm{MNAR}&:&\mathrm{logit}(\Pr(R_2=0)) = Y_2\end{aligned}$$ where
$\mathrm{logit}(p)=\log(p/(1-p))$ for any $0 < p < 1$ is the logit
...
![Distribution of $Y_\mathrm{obs}$ and $Y_\mathrm{mis}$ under three
missing data models.<span data-label="fig:mcarmar">](fig/ch2_marplot-1){width="\maxwidth"}

Figure \[fig:mcarmar\] displays the distribution of $Y_\mathrm{obs}$ and
...
example, in the MAR example of Section \[sec:MCARreprise\] the
...
\[sec:whenignorable\] for more details. In the above example, overweight
...
Section \[sec:nonignorableoverview\] for a more detailed discussion of
...
$$E(\hat Q|Y) = Q  \label{eq:unbiasedness}$$ The explanation of
confidence validity requires some additional symbols. Let $U$ be the
estimated variance-covariance matrix of $\hat
Q$. This estimate is *confidence valid* if the average of $U$ over all
possible samples is equal or larger than the variance of $\hat Q$. The
formula is $$E(U|Y) \geq V(\hat Q|Y) \label{eq:confidenceproper}$$ where
the function $V(\hat Q|Y)$ denotes the variance caused by the sampling
...
$$P(Q|{\mbox{$Y_\mathrm{obs}$}}) = \int{P(Q|{\mbox{$Y_\mathrm{obs}$}},{\mbox{$Y_\mathrm{mis}$}})P({\mbox{$Y_\mathrm{mis}$}}|{\mbox{$Y_\mathrm{obs}$}})} d{\mbox{$Y_\mathrm{mis}$}}\label{eq:Qdecomp}$$
...
The interpretation of Equation \[eq:Qdecomp\] is most conveniently done
...
\[eq:Qdecomp\] says that the actual posterior distribution of $Q$ is
...
\label{eq:poolQ}$$ where $\hat Q_\ell$ contains $k$ parameters and is
...
$$V(Q|{\mbox{$Y_\mathrm{obs}$}})=E[V(Q|{\mbox{$Y_\mathrm{obs}$}},{\mbox{$Y_\mathrm{mis}$}})|{\mbox{$Y_\mathrm{obs}$}}] + V[E(Q|{\mbox{$Y_\mathrm{obs}$}},{\mbox{$Y_\mathrm{mis}$}})|{\mbox{$Y_\mathrm{obs}$}}]\label{eq:varDecomp}$$
...
Equation \[eq:varDecomp\] suggests the following procedure to estimate
$T_\infty$ for finite $m$. We calculate the average of the complete-data
variances as $$\bar U = \frac{1}{m}\sum_{\ell=1}^m \bar U_\ell
  \label{eq:within}$$ where the term $\bar U_\ell$ is the
...
$$B = \frac{1}{m-1}\sum_{\ell=1}^m (\hat Q_\ell-\bar Q)(\hat Q_\ell-\bar Q)'
  \label{eq:between}$$ where $\bar Q$ is calculated by Equation
\[eq:poolQ\].
...
$$\begin{aligned}
  T & = & \bar U + B + B/m \nonumber \\
      & = & \bar U + \left(1+\frac{1}{m}\right)B\label{eq:poolT}\end{aligned}$$
...
results by Equations \[eq:poolQ\] and \[eq:poolT\] is referred to as
Rubin’s rules.
...
advice is to set $m$ higher, e.g., $m=50$ (cf.Section \[sec:howmany\]).
...
\[sec:migoal\] described two conditions needed for a valid estimate of
...
\[sec:migoal\], the whole procedure is valid [@RUBIN1987 p. 119].
...
Recall from Section \[sec:migoal\] that the goal of multiple imputation
...
$(\hat Q, U)$ at the sample level. Table \[tab:misymbols\] provides an
...
following conditions hold approximately: $$\begin{aligned}
  E(\bar Q|Y) &=& \hat Q \label{eq:proper1} \\
  E(\bar U|Y) &= & U \label{eq:proper2} \\
  \left(1+\frac{1}{m}\right) E(B|Y) & \geq & V(\bar Q) \label{eq:proper3}\end{aligned}$$
...
variance due to missing data. Equation \[eq:proper3\] implies that the
...
\[eq:confidenceproper\] for confidence valid estimates of $U$.

If we replace $\geq$ in Equation \[eq:proper3\] by $>$, then the
...
Section \[sec:evaluation\] describes simulation-based tools for checking
the adequacy of imputations for valid statistical inference. Chapter
\[ch:univariate\] provides examples of proper and improper procedures.
...
$$\lambda = \frac{B+B/m}{T} \label{eq:lambda}$$ can be interpreted as
...
The ratio $$r = \frac{B+B/m}{\bar U}$$ is called the *relative increase
...
defined by $$\gamma = \frac{r+2/(\nu+3)}{1+r}\label{eq:gammama}$$ This
measure needs an estimate of the degrees of freedom $\nu$, and will be
discussed in Section \[sec:df\]. The interpretations of $\gamma$ and
...
$$\bar\lambda = \left(1+\frac{1}{m}\right)\mathrm{tr}(BT^{-1})/k \label{eq:barlambda}$$
where $k$ is the dimension of $\bar Q$, and where $B$ and $T$ are now $k
\times k$ matrices. The compromise expression for $r$ is equal to
$$\bar r = \left(1+\frac{1}{m}\right)\mathrm{tr}(B\bar U^{-1})/k \label{eq:barrm}$$
...
degrees of freedom can be written concisely as $$\begin{aligned}
  \nu_\mathrm{old} & = & (m-1)\left(1+\frac{1}{r^2}\right)  \nonumber\\
  & = & \frac{m-1}{\lambda^2}\label{eq:oldnu}\end{aligned}$$ with $r$
and $\lambda$ defined as in Section \[sec:varianceratios\]. The lowest
...
@BARNARD1999A noted that Equation \[eq:oldnu\] can produce values that
...
imputation can be written concisely as $$\nu =
  \frac{\nu_\mathrm{old} \nu_\mathrm{obs}}
  {\nu_\mathrm{old}+\nu_\mathrm{obs}} \label{eq:newnu}$$ The quantity
$\nu$ is always less than or equal to $\nu_\mathrm{com}$. If
$\nu_\mathrm{com}=\infty$, then Equation \[eq:newnu\] reduces to
\[eq:oldnu\]. If $\lambda=0$ then $\nu=\nu_\mathrm{com}$, and if
...
the `pool()` function in
`mice`. The following code
...
as defined in Equation \[eq:poolQ\]. Columns
`ubar`, `b` and
`t` are the variance estimates from Equations
\[eq:within\], \[eq:between\] and \[eq:poolT\], respectively. Column
`dfcom` is the degrees of freedom is the
...
Section \[sec:singlepar\] provides the relevant statistical procedures.
...
therefore deferred to Section \[sec:pooling\]. The next section shows
...
Equations \[eq:proper1\]–\[eq:proper3\] describe the conditions needed
...
\[eq:poolT\]. See @VINK2014 for a short exploration of the idea.
...
\[sec:true\] that it is not a suitable metric to evaluate multiple
imputation methods.

### Example {#sec:quantifyingbias}

This section demonstrates the measures defined in Section
\[sec:evaluationcriteria\] can be calculated using simulation. The
...
imputation methods: regression imputation (cf. Section \[sec:regimp\])
and stochastic regression imputation (cf. Section \[sec:sri\]).
...
statistics as defined in Section \[sec:evaluationcriteria\].
...
\[ch:univariate\] shows how to make it fully randomization-valid.
...
$$\mathrm{RMSE} = \sqrt{\frac{1}{n_\mathrm{mis}}\sum_{i=1}^{n_\mathrm{mis}} ({\mbox{$y_i^\mathrm{mis}$}}- {\mbox{$\dot y_i$}})^2} \label{eq:recovery}$$
...
recreating the missing data. Remember from Section \[sec:regimp\] that
...
Section \[sec:doesnotcover\] already discussed some approaches not
...
Section \[sec:doesnotcover\], and may work well in particular
...
depends on $\nu$, and thus on $m$ (cf. Equation \[eq:oldnu\]). He
...
a quadratic rule $$\label{eq:hippel}
m = 1 + \frac{1}{2} \left(
         \frac{\gamma_0}
...
is then plugged into Equation \[eq:hippel\]. Compared to Bodner, the
...
1.  *Nomogram*. Construct a graphic representation of Equation
    \[eq:gammamb\] that allows the user to convert $\lambda$ and
...
5.  *Number of imputations (continued)*. Continue with the data from the
    ...
    3.  Check White’s conditions 1 and 2 (cf. Section \[sec:howmany\]).
...
6.  *Automated choice of $m$*. Write an
    ...
    the methods discussed in Section \[sec:howmany\].
