...
\@ref(sec:missmult) reviews several non-imputation approaches for dealing
with missing values in multilevel data. Sections \@ref(sec:mljoint) and
\@ref(sec:mlfcs) describe imputation using the joint modeling and fully
conditional specification frameworks. Sections \@ref(sec:multioutcome) and
\@ref(sec:catoutcome) review current procedures for imputation under
multilevel models with continuous and discrete outcomes, respectively.
Section \@ref(sec:level2pred) deals with missing data in the level-2
predictors, and Section \@ref(sec:comparative) summarizes comparative work
on the different approaches. Section \@ref(sec:mlguidelines) contains
...
*composite notation* as $$(\#eq:composite2)
...
$$y_c = X_c\beta + Z_cu_c + \epsilon_c  (\#eq:lmm)$$ where
...
within-group part as $$(\#eq:jm.decomp)
...
Because of the decomposition \@ref(eq:jm.decomp), imputation by joint
...
values in `lpo` and
`sex` in model \@ref(eq:composite2). One way to
...
\@ref(eq:naivefcs) does not account for the cluster means or for the
...
\@ref(sec:congeniality) explains the confusion surrounding the term
...
Imputations under the linear mixed model in Equation \@ref(eq:lmm) can be
...
Table \@ref(tab:mlacontinuous) presents an overview of
...
Section \@ref(sec:threeformulations). Here we will analyze the full set of
...
\@ref(fig:mlsd) shows that the standard deviation per school varies between
...
Figure \@ref(fig:mldens) shows the density plot of the 10 sets of imputed
...
`micemd` and
`countimp` packages. Table \@ref(tab:mlacat)
...
Figure \@ref(fig:toenail.profiles) visualizes the imputations. The plot
...
Section \@ref(sec:ri1pred) contains an example of that problem.
...
suitable methods from Chapter \@ref(ch:univariate) to impute the missing
...
of clustering. Table \@ref(tab:funcmixed) lists the various methods.
...
Figure \@ref(fig:mla.mdp), with the missing data patterns, reveals that
...
\@ref(sec:cont.exam). Here we select the imputations according to the
...
imputation of blocks of variables (cf. Section \@ref(sec:blockvar)). The
final estimates on the multiply imputed data under model \@ref(eq:r1) can
...
We continue with the analysis of Section \@ref(sec:ri1pred). We extend the
...
imputation model of Section \@ref(sec:ri1pred). Thus, we may use the same
...
the methods listed in Table \@ref(tab:funcmixed). It is not yet clear which
...
`iqv` and `den`. The
difference with model \@ref(eq:r2) is that `den`
...
variable can be a level-1 aggregate (e.g., as in Section \@ref(sec:wbg)),
or a measured level-2 variable (as in Section \@ref(sec:ril2)). Missing
...
\@ref(sec:clustermeans) for more details.
...
IQ (cf. Example 5.1 in @SNIJDERS2012). Section \@ref(sec:wbg) showed how to
...
Section \@ref(sec:ri1pred), but now including both the cluster means and
...
model \@ref(eq:rs2level). According to the model, both cluster means and
...
done in Section \@ref(sec:mlint). This circumvents many of the
...
Table \@ref(tab:recipes) contains two recipes for imputing multilevel data.
...
predictor specification strategy in Section \@ref(sec:predictors) to
...
Sections \@ref(sec:emptymodel) 
to \@ref(sec:rs.interactions) demonstrated
...
The procedure in Section \@ref(sec:rs.interactions) may be a daunting task
...
\@ref(sec:modelbased) for some pointers into the literature.
