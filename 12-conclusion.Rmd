...
reporting guidelines for applications. Section \@ref(sec:otherapps) gives
an overview of applications that were omitted from the book. Section
\@ref(sec:future) contains some speculations about possible future
developments.
...
2.  *Reasons for missingness:* What is known about the reasons for
    ...
    related to other variables in the study?\@ref(lis:reasons)
...
3.  *Consequences:* Are there important differences between individuals
    ...
    complete-case analysis is used?\@ref(lis:differences)
...
9.  *Diagnostics:* How has convergence been monitored? How do the
    ...
    been missing? \@ref(lis:diagnostics)
...
11. *Complete-case analysis:* Do multiple imputation and complete-case
    ...
    explain the difference?\@ref(lis:listwise)
...
12. *Sensitivity analysis:* Do the variables included in the imputation
    ...
    nonignorable model?\@ref(lis:sensitivity)

If space is limited, the main text can be restricted to a short summary
of points \@ref(lis:amount), 
\@ref(lis:reasons), 
\@ref(lis:method),
\@ref(lis:software), 
\@ref(lis:number) and 
\@ref(lis:listwise), whereas the
remaining points are addressed in a appendix or online supplement.
Section \@ref(sec:template) contains an example template.

For clinical trials, reporting in the main text should be extended by
point \@ref(lis:sensitivity), conform to recommendation 15 of @NRC2010.
...
Chapters \@ref(ch:measurement)–
\@ref(ch:longitudinal) illustrated several
...
information. Sections \@ref(sec:convergence) 
and \@ref(sec:impbridge) put this
...
1.  *Do’s:* Take the list of do’s in Section \@ref(sec:dos). For each item
