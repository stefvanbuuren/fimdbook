...
Chapter \[ch:univariate\] dealt with univariate missing data. In
...
defined in Section \[sec:notation\].
...
Figure \[fig:patterns\] illustrates various data patterns in
...
All patterns displayed in Figure \[fig:patterns\] are connected. The
...
\[fig:patterns\] it is still connected through the first two rows.
...
format” (cf. Section \[sec:longandwide\]) are more complex than the
patterns in Figure \[fig:patterns\]. See @VANBUUREN2011 [p. 179] for
some examples.
...
`pattern4` in Figure \[fig:patterns\] is
...
For example, for the monotone pattern in Figure \[fig:patterns\] the
frequencies are:
...
dataset `pattern4` in Figure \[fig:patterns\]
...
`pattern4` in Figure \[fig:patterns\] for all
pairs $(Y_j, Y_k)$ by
...
Figure \[fig:patterns\]. The influx of a variable quantifies how well
...
of observed data, $I_j$ and $O_j$. Figure \[fig:fluxexample\] shows the
influx-outflux pattern of the four patterns in Figure \[fig:patterns\]
...
methods as discussed in Chapter \[ch:univariate\] can be used as
...
Algorithm \[alg:monotone\] provides the main steps of monotone data
...
executed by one of the univariate methods of Chapter \[ch:univariate\].
...
will explore this technique in more detail in Section \[sec:FCS\], but
...
Algorithm \[alg:joint\] lists the major steps needed to impute
...
covered in a Chapter \[ch:multilevel\].
...
Section \[sec:mljoint\] discusses these developments in more detail.
...
methods of Chapter \[ch:univariate\] may act as building blocks. FCS is
...
specified models. Algorithm \[alg:mice\] describes one particular
...
imputations by executing Algorithm \[alg:mice\] in parallel $m$ times.
...
\[sec:compatibility\]), the MICE algorithm is a Gibbs sampler, a
...
applications of the method (cf. Section \[sec:compatibility\]).
...
See Section \[sec:convergence\] for an example. As long as the
...
eliminate feedback loops from the system. Chapter \[ch:practice\]
...
to fit the data. See Section \[sec:compatibility\] for more detail.
...
only memory in the the MICE algorithm. Chapter \[ch:univariate\]
...
Figure \[fig:conv\] shows the development of $\rho(Y_1,Y_2)$ calculated
...
Figure \[fig:jm\] plots the results of the first five imputations from
...
\[fig:jm\] is larger than the observed data range. The observed data are
...
Imputations for the proportonal odds model in Figure \[fig:fcs\] differ
markedly from those in Figure \[fig:jm\]. This model yields imputations
...
\[fig:jmfcs\] indicate the model fitted on the complete cases, whereas
the thin black lines correspond to the analyses of the 10 imputed
datasets.

The different panels of Figure \[fig:jmfcs\] corresponds to different
...
predictive mean matching* applied Algorithm \[alg:pmm\] as a component
...
\[sec:categorical\]. The imputed data essentially agree with the
complete-case analysis, perhaps apart from some minor deviations around
the probability level of 0.9.

Figure \[fig:jmfcs\] shows clear differences between FCS and JM when
...
See Section \[sec:jmcategorical\]. @HORTON2003 expected that bias
...
The MICE algorithm listed in box \[alg:mice\] can be extended in several
ways.
...
weakness of the algorithm in box \[alg:mice\] is that it may become
...
multivariate imputation. The methods in Section \[sec:blockvar\]
partition the columns and apply one model to many outcomes, whereas the
methods in Section \[sec:blockunit\] partition the rows and apply many
...
2.  *Convergence*. Figure \[fig:conv\] shows that convergence can take
    ...
    1.  The default argument `ns` of the
        ...
        \[sec:slowconvergence\] defines six scenarios with different
    ...
    3.  Change the scenario in which all data in $Y_1$ and $Y_2$ are
        ...
        and create a figure similar to Figure \[fig:conv\].

    4.  Compare your figure with Figure \[fig:conv\]. Are there any
...
3.  *Binary data*. Perform the simulations of Section
    \[sec:slowconvergence\] with binary $Y_1$ and $Y_2$. Use the odds
