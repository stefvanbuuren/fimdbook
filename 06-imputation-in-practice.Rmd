...
Chapters \[ch:univariate\] and \[ch:multivariate\] describe methods to
...
imputations (cf. Section \[sec:proper\]), and thus result in valid
...
1.  First, we should decide whether the MAR assumption is plausible. See
    Sections \[sec:MCAR\] and \[sec:MCARreprise\] for an introduction to
    ...
    \[sec:nonignorable\] described one way to do so within the
    FCS framework. Section \[sec:whenignorable\] deals with this issue
    in more detail.
...
2.  The second choice refers to the form of the imputation model. The
    ...
    \[ch:univariate\] described many different methods for creating
...
3.  A third choice concerns the set of variables to include as
    ...
    model specifications. Section \[sec:modelform\] describes the
...
4.  The fourth choice is whether we should impute variables that are
    ...
    into the multiple imputation algorithm. Section \[sec:knowledge\]
...
5.  The fifth choice concerns the order in which variables should
    ...
    \[sec:visit\] discusses relevant options.
...
6.  The sixth choice concerns the setup of the starting imputations and
    ...
    can be monitored in many ways. Section \[sec:convergence\] outlines
...
7.  The seventh choice is $m$, the number of multiply imputed datasets.
    ...
    information is high. Section \[sec:howmany\] provided guidelines for
    setting $m$.
...
Section \[sec:diagnostics\] highlights some diagnostic tools that aid in
determining the choices.

## Ignorable or nonignorable? {#sec:whenignorable}

Recall from Section \[sec:ignorability\] that the assumption of
...
$Z$ (like MARRIGHT in Section \[sec:generateuni\]), then omitting $Z$
...
nonignorable imputation model (cf. Section \[sec:nonignorable\]) to
...
Section \[sec:sensitivity\] contains an example of how this can be done
in practice.
...
\[ch:univariate\] discussed many possible methods. The measurement level
...
Table \[tab:elem\] lists the built-in univariate imputation method in
...
\[sec:flux\]. The `mice()` function detects
...
`chl` and `chl2`,
their relation is not maintained. See Figure \[fig:chlchl\].
...
parameter, denoted by $\kappa$ in Algorithms \[alg:norm\],
\[alg:normboot\] and \[alg:pmm\]. The `ridge`
...
*JAV*. Figure \[fig:passive\] visualizes the consistency of the three
methods.
...
Table \[tab:simratio\] shows that all three methods have substantial
...
The results of the simulations are also in Table \[tab:simratio\] under
...
Figure \[fig:interaction\] illustrates that the scatterplots of the real
...
\[tab:siminteraction\] contains the results of simulations similar to
those in Section \[sec:ratio\], but adapted to include the interaction
effect shown in Figure \[fig:interaction\] by using the complete-data
...
and classification (Section \[sec:cart\]) as well as various joint
modeling methods (Section \[sec:JM\]). The relative strengths and
...
$$X=\alpha + Y\beta_1 + Y^2\beta_2 + \epsilon \label{eq:quadratic}$$
...
can impute the missing part of $Z$ by Algorithm \[alg:norm\]. In cases
...
into $Y$ and $Y^2$. Under the model in Equation \[eq:quadratic\] the
value $Y$ has two roots:

$$\begin{aligned}
Y_- &=- { 1 \over 2\beta_2 }  \left( \sqrt{4 \beta_2 Z + \beta_1^2} + \beta_1 \right)\label{eq:leftroot}\\
Y_+ &=  { 1 \over 2\beta_2 }  \left( \sqrt{4\beta_2 Z + \beta_1^2} - \beta_1 \right)\label{eq:rightroot}\end{aligned}$$
...
Algorithm \[alg:squares\] provides a detailed overview of all steps
...
Figure \[fig:composition\] shows where the solution might be further
...
conditional on other data. The method in Section \[sec:sri\] produced
...
Compare Figure \[fig:squeeze\] to Figure \[fig:sri\]. The negative ozone
...
Figure \[fig:gen\] compares the scatterplot of genital development
...
mechanisms (see Section \[sec:nonignorable\]). More generally, we might
...
\[sec:interactions\]. The visit sequence is
...
which produces Figure \[fig:converge\]. On convergence, the different
...
`bmi` feed back into
`hgt` and `wgt`.
Figure \[fig:convergencesteadystate\] shows that the streams hardly mix
...
Figure \[fig:convergencehealthy\] is the resulting plot for the same
...
\[sec:slowconvergence\] (Pearson correlation) and
\[sec:walkingimputation\] (Kendall’s $\tau$) for examples.
...
Figure \[fig:wormpmm\] is the worm plot calculated from imputed data
...
-   the points have different means (Figure \[fig:mcarmar\]);

-   the points have different spreads (Figures \[fig:meanimp\],
    \[fig:regimp\] and \[fig:locf\]);

-   the points have different scales (Figure \[fig:jm\]);

-   the points have different relations (Figure \[fig:passive\]);
...
to obtain stability. Figure \[fig:propensity\] plots BMI against the
...
-   Check the modeling choices in Section \[sec:choices\]. Though the
...
-   Use MAR as a starting point using the strategy outlined in
    Section \[sec:whenignorable\].

-   Choose the imputation methods and set the predictors using the
    strategies outlined in Section \[sec:modelform\].
...
-   Use passive imputation if you need the derived variables
    ...
    feedback loops. See Section \[sec:knowledge\].
...
-   Monitor convergence of the MICE algorithm for aberrant patterns,
    ...
    dependencies in the data. See Sections \[sec:howlarget\]
    and \[sec:convergence\].
...
1.  *Worm plot for normal model*. Repeat the imputations in Section
    \[sec:fitversus\] using the linear normal model for the
    ...
    -   Are there striking differences between your worm plot and
        Figure \[fig:wormpmm\]? If so, describe.
